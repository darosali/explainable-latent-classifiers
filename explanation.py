...
def find_knn_latent(x, X_train_n, z_train_n, y_train_n, X_train_a, z_train_a, y_train_a, k=9):
    knn_normal = NearestNeighbors(n_neighbors=k, algorithm='auto', metric='euclidean')
...
def show_importance(coefficients, feature_names, n_features=10):
    ...
    print(f"num of non-zero features: {feature_importance[feature_importance != 0].shape[0]}")
